---
description: >-
  Stops an internal `task` subagent call from silently substituting for BATMAN
...
Stop this call. Use the `batman-orchestration` skill and BATMAN's own tools
(`batman_worker`, `batman_task`, `batman_run`) to satisfy this request instead of `task`.
