...
| `docs/` | Engineering documentation (start with `getting-started.md`, `architecture.md`) |
...
- **Validation:** Every message from the daemon is validated with Ajv before reaching extension logic
...
2. **TypeScript validates every message** from the daemon with Ajv before extension logic processes it.
