...
**Location:** `crates/runtime/src/service/orchestration.rs:341-351` (`task_upsert`'s pre-check); `crates/runtime/src/service/orchestration.rs:1850-1860` (`reconcile_omp`'s pre-check); `crates/runtime/src/domain/repository.rs:310-357` (`upsert_task`'s `ON CONFLICT` write, unguarded by revision; its doc comment at `:307-309` delegates the invariant to the caller); `crates/runtime/src/domain/repository.rs:1282` (`reconcile_ownership`'s `UPDATE`, unguarded by revision)

**Evidence:** both `task_upsert` (`orchestration.rs:343-351`) and `reconcile_omp` (`orchestration.rs:1850-1860`) read the stored revision via one `run_domain_op` round trip (`query::task_get_op`), compare it to the caller-supplied revision entirely in memory, and only then issue a second `run_domain_op` call that performs the write. Neither `upsert_task`'s `ON CONFLICT(task_id) DO UPDATE SET ... revision = excluded.revision` (`repository.rs:343-346`) nor `reconcile_ownership`'s `UPDATE tasks SET ... revision = ?2 ... WHERE task_id = ?4` (`repository.rs:1282`) carries a revision predicate — both write unconditionally once they reach the transaction, and `upsert_task`'s own doc comment (`repository.rs:307-309`) explicitly delegates "a lower revision is rejected" to the caller. Between a `task_upsert`'s read and its write, a concurrent `task_upsert` or `reconcile_omp` can commit; two concurrent `task_upsert` calls carrying revisions N and N-1 can both pass their independent pre-checks (each compares against the same stale `stored_revision`) and then commit in either order — if the one carrying N-1 lands last, `tasks.revision` moves backwards and `owner_client_instance_id` rebinds to whichever client supplied the lower revision. Symmetrically, two concurrent `reconcile_omp` calls presenting the same revision — the exact reconnect-storm scenario the mechanism exists for — both read `stored_revision`, both pass the `revision != stored_revision` check, and both then rebind ownership, so the second caller's principal wins the race; that breaks `reconcile_omp`'s own doc promise ("only when task ID and monotonic OMP revision match", `orchestration.rs:1839-1841`), since the second call's in-memory comparison is already stale by the time the first reconcile has landed. Since R71/R72 made `tasks.owner_client_instance_id` the in-transaction authority that `resolve_policy_violation` and `decide_approval` trust for their ownership checks, those checks are only as trustworthy as this write: a lost update here silently hands a stale client the authority those in-transaction checks rely on.

**Fix:** mirror R70-R72's fix — move the predicate into the write itself (an `AND revision <= ?` guard on `upsert_task`'s conflict-path `UPDATE`, an `AND revision = ?` guard on `reconcile_ownership`'s `UPDATE`), classify zero affected rows inside the same transaction into a typed `DomainError`, map it to the existing caller-error class at the service layer, and delete both caller-side pre-checks (`orchestration.rs:341-351`, `:1850-1860`) once the in-transaction check subsumes them.

**Priority:** High — found during R72's adversarial review, 2026-08-18; same check-then-act-across-two-round-trips shape as R70-R72, but on the write that binds task ownership and revision, which the ownership checks those findings fixed now depend on being correct.

#### R75. Quarantine state is decided from caller-side snapshots across separate commits

**Location:** `crates/runtime/src/policy/violation.rs:249,309` (`already_actioned = flags.policy_quarantined || state.is_terminal()`, read one round trip before `record_policy_violation`); `:355-357` (`apply_action`'s short-circuit consuming that stale read); `:557-571` (`resolve_policy_violation`'s commit) and `:600` (`decide`'s release path issuing `set_quarantined(run_id, false)` as a second, separate commit); secondary location `crates/runtime/src/service/orchestration.rs:290-298` (`ensure_not_quarantined`'s own-round-trip flag read, acted on by the caller in a later round trip)

**Evidence:** `record_nested_worker`/`record_cost_ceiling` read `already_actioned` in their own round trip (`violation.rs:249`, `:309`) before the same call's `record_policy_violation` write commits; `apply_action`'s `if already_actioned { return Ok(()); }` (`:355-357`) trusts that stale read. A `decide` release committing between the read and `record_policy_violation`'s commit leaves `already_actioned` stale, so the freshly-journaled violation never applies quarantine. The mirror interleaving: `decide`'s release path commits `resolve_policy_violation` (`:557-571`) and `set_quarantined(run_id, false)` (`:600`) as two separate commits — a new violation quarantining the run in between is immediately un-quarantined by the release's second commit. Both interleavings end with a journaled, unresolved violation on a run whose `policy_quarantined` is false — the exact silently-un-quarantining-a-violating-run harm R73's own priority line cites. Reachable because `apply_action` is driven by the adapter event stream (`event_sink.rs`) while `decide` is a separate operator RPC — genuinely concurrent callers. A secondary instance of the same shape: `ensure_not_quarantined` (`orchestration.rs:290-298`) reads `flags['policyQuarantined']` in its own `run_domain_op` round trip, then the caller acts on the result in a later, separate round trip — a run quarantined between the check and the gated write is not stopped. Found during R73's adversarial review (`agent://R73Adversary` W2, S3); confirmed not already registered — `REVIEW.md` had no `already_actioned`/check-then-act entry for this shape, and is disjoint from R74's task-revision scope.

**Fix:** mirror the R70-R74 doctrine — move idempotency/consistency arbitration into guarded writes instead of caller-held snapshots. Have `record_policy_violation` re-read `flags.policy_quarantined` and run state inside its own transaction and return a classified discriminator (already-actioned vs. newly-actioned) for `apply_action` to branch on; make the release path's un-quarantine unable to clobber a quarantine journaled after `resolve_policy_violation`'s commit (e.g. a guarded `UPDATE ... WHERE flags_policy_quarantined = ?` keyed on the pre-release value, or folding both writes into one transaction). Delete the caller-side `already_actioned` pre-check in `record_nested_worker`/`record_cost_ceiling`, and fold `ensure_not_quarantined`'s pre-check into the same guarded-write point of use rather than a separate advance check.
