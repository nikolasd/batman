...
impl LeaseService {
    ...
    pub fn get(&self, lease_id: String) -> Result<WorkspaceInfo, LeaseError> {
        ...
        let mode = match mode_str.as_str() {
            ...
            _ => return Err(LeaseError::NotFound { lease_id: lease_id.clone() }),
        };
    ...
    pub fn release(&self, lease_id: String) -> Result<(), LeaseError> {
        ...
        conn.execute("UPDATE workspace_leases SET state = 'released', released_at = ?1 WHERE lease_id = ?2", params![now_str, lease_id])
            .map_err(|e| LeaseError::Db(e.to_string()))?;
...
fn run_id_from_str(s: &str) -> Result<RunId, LeaseError> {
    RunId::parse(s).map_err(|_| LeaseError::NotFound { lease_id: s.to_string() })
}